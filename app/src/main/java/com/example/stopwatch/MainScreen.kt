    ...
    Column(
            ...
            AnimatedContent(targetState = hours, transitionSpec = { addAnimation() }) {
                Text(
                    ...
                    style = TextStyle(
                        ...
                        color = if (hours == "00") Color.White else Color.Red
                    )
                )
            }
            ...
            AnimatedContent(targetState = seconds, transitionSpec = { addAnimation() }) {
                Text(
                    text = seconds, style = TextStyle(
                        fontSize = MaterialTheme.typography.h1.fontSize,
                        fontWeight = FontWeight.Bold,
                        color = if (seconds == "00") Color.White else Color.DarkGray
                    )
                )
            }
        }
        Row(modifier = Modifier.weight(weight = 1f)) {
            ...
            Button(
                ...
                colors = ButtonDefaults.buttonColors(disabledBackgroundColor = Color.Unspecified)
            ) {
                Text(text = "Cancel")
            }
        }
